...
import ru.volga.launcher.other.Interface;
...
public class SplashFragment extends BaseFragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        scall.enqueue(new Callback<List<Servers>>() {
            ...
            public void onResponse(Call<List<Servers>> call, Response<List<Servers>> response) {
                ...
                for (Servers server : servers) {
                    Lists.slist.add(new Servers(server.getVersion(), server.getColor(), server.getname(), server.getOnline(), server.getmaxOnline(), server.getRecommend(), server.getHost(), server.getPort(), server.getId()));
                }
            }

            @Override
            public void onFailure(Call<List<Servers>> call, Throwable t) {
                Toast.makeText(getContext(), "Не удалось прогрузить сервера. Проверьте интернет соединение!", Toast.LENGTH_SHORT).show();
            }
        });
        ...
        ncall.enqueue(new Callback<List<News>>() {
            ...
            public void onFailure(Call<List<News>> call, Throwable t) {
                Toast.makeText(getContext(), "Не удалось прогрузить новости. Проверьте интернет соединение!", Toast.LENGTH_SHORT).show();
            }
        });
        return inflate;
    }
    ...
    private void loadAPI() {
        ApiService.getInstance().getApiService().getLinks().enqueue(new Callback<Links>()
        {
                ...
                if (response.isSuccessful()) {
                    try {
                        if (response.body() != null) {
                            ApiService.getInstance().version_launcher=response.body().getVersionLauncher();
                            ApiService.getInstance().version_modpack=response.body().getVersionModPack();
                            ApiService.getInstance().url_cache=response.body().getUrlFiles();
            ...
            public void onFailure(Call<Links> call, Throwable th) {
                Toast.makeText(getContext(), "Ошибка загрузки данных. Проверьте интернет соединение!", Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    /*private void pangpang()
    {
        ...
        t.schedule(new TimerTask(){
            ...
            @Override
            public void run() {
                splash_logo.startAnimation(AnimationUtils.loadAnimation(getContext(), R.anim.scale2));
                //splash_logo.startAnimation(AnimationUtils.loadAnimation(getContext(), R.anim.scale));
                //splash_logo.Animations(R.anim.mtrl_bottom_sheet_slide_in, R.anim.mtrl_bottom_sheet_slide_out);
                pangpang2();
            }
        }, 600L);
    }
    private void pangpang2()
    {
        Timer t = new Timer();
        t.schedule(new TimerTask(){
...
//splash_logo.animate().alpha(0.0f).setDuration(150); /*ru.volga.online.gui.keyboard.k5.c.j(this.f8704b, 0.0f, 150);*/
}
