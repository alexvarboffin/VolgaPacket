...
/*const cryptor::string_encryptor encrArch[MAX_ENCRYPTED_TXD] = { 
			cryptor::create("texdb/samp/samp.txt", 21), 
			cryptor::create("texdb/gui/gui.txt", 19) 
};*/
const cryptor::string_encryptor encrArch[MAX_ENCRYPTED_TXD] = {
...
void InitialiseRenderWare_hook()
{
	...
	(( void (*)(const char*, int, int))(g_libGTASA+0x1BF244+1))("mobile", 0, 5);
	(( void (*)(const char*, int, int))(g_libGTASA+0x1BF244+1))("txd", 0, 5);
	(( void (*)(const char*, int, int))(g_libGTASA+0x1BF244+1))("gta3", 0, 5);
	(( void (*)(const char*, int, int))(g_libGTASA+0x1BF244+1))("gta_int", 0, 5);
	(( void (*)(const char*, int, int))(g_libGTASA+0x1BF244+1))("menu", 0, 5);
...
void CTaskComplexLeaveCar_hook(uintptr_t** thiz, VEHICLE_TYPE *pVehicle, int iTargetDoor, int iDelayTime, bool bSensibleLeaveCar, bool bForceGetOut) 
{
	...
	dwRetAddr -= g_libGTASA;
 	...
 	if (dwRetAddr == 0x3AE905 || dwRetAddr == 0x3AE9CF) 
 	{
 		if (pNetGame) 
 		{
             //Добавь поддержку 'armeabi-v7a', 'x86_64', 'arm64-v8a'


//             //Мы знаем uint32_t pVehicle;			// 1420-1424	;pVehicle
//
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
...
void MobileMenu__Render_hook(uintptr_t thiz)
{
	MobileMenu__Render(thiz);
	 const char/*jstring*/ *ponel = OBFUSCATE("VOLGA MAP");
...
{	
	NvUtilInit();
	*(char**)(g_libGTASA + 0x5D1608) = "/storage/emulated/0/VolgaOnline/";
}
