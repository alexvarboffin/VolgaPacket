...
{
	strcpy(m_szHostName, "Volga Online Start..");
...
void CNetGame::UpdateNetwork()
{
	...
	while(pkt = m_pRakClient->Receive())
	{
		...
		switch(packetIdentifier)
		{
			...
			g_pJavaWrapper->UpdateSplash(101, 2);
...
#define RPC_SHOW_NAMEID	0x26
#define RPC_FOOT_DATE	0x27
#define RPC_CUSTOM_SET_MILEAGE		0x28
#define RPC_CUSTOM_SET_EAT		0x29
#define RPC_FOOT_SERVER	0x30
...
#define RPC_SHOW_HAPPY	0x186
#define RPC_HIDE_HAPPY	0x187
#define RPC_SHOW_ALOGIN	0x188
#define RPC_HIDE_ALOGIN	0x189
#define RPC_SHOW_WORK	0x190
#define RPC_HIDE_WORK	0x191
#define RPC_SHOW_ADMREPS	0x192
#define RPC_HIDE_ADMREPS	0x193
#define RPC_SHOW_ADMASK	0x194
#define RPC_HIDE_ADMASK	0x195
...
void CNetGame::Packet_CustomRPC(Packet* p)
{
	...
	case RPC_FOOT_SERVER:
	{
		char szBuff[128+1];
		char text[64*54];
		char textBtn[64*54];
		char actionBtn2[64*54];
		//текст
		uint16_t lenText;
		bs.Read(lenText);
		...
		g_pJavaWrapper->SetFootServer(text);
		break;
	}
	case RPC_FOOT_DATE:
	{
		...
		g_pJavaWrapper->SetFootDate(text);
		break;
	}
	case RPC_SHOW_NAMEID:
	{
		char szBuff[128+1];
		char text[64*54];
		char textBtn[64*54];
		...
		g_pJavaWrapper->SetShowNameID(text);
		break;
	}
	case RPC_SHOW_ZONA:
	{
	...
	case RPC_SHOW_WORK:
	{
		...
		g_pJavaWrapper->ShowWork(text);
		break;
	}
	case RPC_HIDE_WORK:
	{
		g_pJavaWrapper->HideWork();
		break;
	}
	case RPC_SHOW_X2:
	{
	...
	case RPC_SHOW_HAPPY:
	{
		g_pJavaWrapper->ShowHappy();
		break;
	}
	case RPC_HIDE_HAPPY:
	{
		g_pJavaWrapper->HideHappy();
		break;
	}
	case RPC_SHOW_HUD:
	{
	...
	case RPC_SHOW_ALOGIN:
	{
		g_pJavaWrapper->ShowAlogin();
		break;
	}
	case RPC_HIDE_ALOGIN:
	{
		g_pJavaWrapper->HideAlogin();
		break;
	}
	case RPC_SHOW_ADMREPS:
	{
		g_pJavaWrapper->ShowAdmReps();
		break;
	}
	case RPC_HIDE_ADMREPS:
	{
		g_pJavaWrapper->HideAdmReps();
		break;
	}
	case RPC_SHOW_ADMASK:
	{
		g_pJavaWrapper->ShowAdmAsk();
		break;
	}
	case RPC_HIDE_ADMASK:
	{
		g_pJavaWrapper->HideAdmAsk();
		break;
	}
	case RPC_SHOW_RECON:
	{
	...
	case RPC_CUSTOM_ACTOR_PUT_IN_VEH:
		...
		case RPC_CUSTOM_SET_EAT :
		{
			float currrent = 0;
			bs.Read(currrent);
			
			pGUI->SetEat(currrent);
			break;
		}
		case RPC_SHOW_NOTIFICATION : 
		{
			...
			uint16_t lenActionBtn2;
			bs.Read(lenActionBtn2);

			memset(actionBtn2, 0, sizeof(actionBtn2));
			memset(szBuff, 0, sizeof(szBuff));

			bs.Read(szBuff, lenActionBtn2);
			cp1251_to_utf8(actionBtn2, szBuff);

			g_pJavaWrapper->ShowNotification(type, text, duration, actionBtn, textBtn, actionBtn2);
			return;
		}
	}
