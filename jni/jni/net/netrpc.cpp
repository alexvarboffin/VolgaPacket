...
#define PRC_VERSION_UPDATE   1401
...
void InitGame(RPCParameters *rpcParams)
{
	...
	bsSend.Write((uint8_t)    251); // 251
	bsSend.Write((uint32_t)    PRC_VERSION_UPDATE); // свой ид любой, главное чтоб одинаковый в клиенте и на сервере
...
void ConnectionRejected(RPCParameters *rpcParams)
{
	...
	else if(byteRejectReason == REJECT_REASON_BAD_NICKNAME)
	{
		...
		pChatWindow->AddInfoMessage(CLocalisation::GetMessage(E_MSG::NIKC));
		pChatWindow->AddInfoMessage(CLocalisation::GetMessage(E_MSG::NIKCC));
		pChatWindow->AddInfoMessage(CLocalisation::GetMessage(E_MSG::NIKCCC));
		pChatWindow->AddInfoMessage(CLocalisation::GetMessage(E_MSG::NIKCCCC));
	}
