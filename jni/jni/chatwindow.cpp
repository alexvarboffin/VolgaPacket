...
void InitBASSFuncs()
{
	...
	handle = dlopen("/data/data/ru.volga.online/lib/libbass.so", 3);
#else
	handle = dlopen("/data/data/ru.volga.online/lib/libbass.so", 3);
#endif
	if (!handle)
...
{
	if (strstr(str, "/alltab"))
	{
		pScoreBoard->Toggle();
		return true;
	}

	if (strstr(str, "/hudmanager"))
	{
...
void CChatWindow::ChatWindowInputHandler(const char* str)
{
	if(!str || *str == '\0') return;
	if(!pNetGame) return;
...
void CChatWindow::AddChatMessage(char* szNick, uint32_t dwNickColor, char* szMessage)
{
	...
	g_pJavaWrapper->AddChatMessage(szMessage);
...
void CChatWindow::AddInfoMessage(char* szFormat, ...)
{
	...
	g_pJavaWrapper->AddChatMessage(tmp_buf);
...
void CChatWindow::AddDebugMessageNonFormatted(char* szStr)
{
	...
	g_pJavaWrapper->AddChatMessage(szStr);
